...
export function setCookie(email) {
  ...
  document.cookie = `MyBills=${email}; timestamp=${timestamp}; path=/; max-age=${expirationTime}`;

  // Check if the cookie is less than 15 days old
  const isCookieValid = checkCookieAge("MyBills");
...
export function removeCookie() {
  // Remove a cookie
  document.cookie = "MyBills=; path=/; max-age=0;";
}
